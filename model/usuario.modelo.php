...
class modeloUsuario {
	...
	static public function mdVerificarToken($tabla,$token,$getToken){
		$stmt=Conexion::conectar()->prepare("SELECT token, estado FROM $tabla WHERE $token=:$token LIMIT 1");
		$stmt->bindParam(":".$token, $getToken,PDO::PARAM_STR);
		$stmt->execute();
		return $stmt->fetch(PDO::FETCH_ASSOC);
	}
	static public function mdActualizarEstadoToken($tabla,$item1,$valor1,$item2,$valor2){
